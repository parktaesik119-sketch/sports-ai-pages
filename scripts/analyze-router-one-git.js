...
HOME_ANALYSIS: (홈팀 분석. 존댓말로 자연스럽게 5문장 이상 서술하라. 반드시 [홈팀 ${seasonLabel} 전체 DB]만 기준으로 ${seasonLabel} 성적(승패수, 승률, 득점 평균 등)을 첫 문장에 언급하고, 최근 흐름과 자연스럽게 연결하라. 득점력, 수비력, 홈/원정 성적, 강점 또는 주목 선수를 흐름 안에 녹여 작성하라. 다른 연도 수치 사용 절대 금지. "최근 5경기에서 N승 N패" 같은 수치 나열식 첫 문장 절대 금지. 축구 종목을 제외하고 나머지 모든 종목은 무승부 표현 절대 금지. 문장 사이 구분은 공백으로만.)
AWAY_ANALYSIS: (원정팀 분석. HOME_ANALYSIS와 동일한 방식으로 원정팀 기준으로 작성하라. 반드시 [원정팀 ${seasonLabel} 전체 DB]만 기준으로 ${seasonLabel} 성적을 먼저 언급하고 최근 흐름과 자연스럽게 연결하라. 다른 연도 수치 사용 절대 금지. 축구 종목을 제외하고 나머지 모든 종목은 무승부 표현 절대 금지. 문장 사이 구분은 공백으로만.)
HOME_POWER: (홈팀 핵심 포인트 5개를 파이프(|)로 구분. 각 20자 이내. 반드시 [홈팀 ${seasonLabel} 전체 DB] 기준 구체적 수치 포함. 예: ${seasonLabel} 26경기 14승12패|평균 4.2득점 공격력 우수|홈 승률 .620 강세|최근 3연승 흐름|수비 실점 2점 이하 유지)
AWAY_POWER: (원정팀 핵심 포인트 5개를 파이프(|)로 구분. 각 20자 이내. 반드시 [원정팀 ${seasonLabel} 전체 DB] 기준 구체적 수치 포함. 예: ${seasonLabel} 26경기 10승16패|평균 3.5득점 수준|원정 2연속 대패|실점 급증 흐름|주전 타선 침묵)
H2H: (상대전적. DB에 있으면 각 경기를 파이프(|)로 구분하여 기재. 형식: YYYY.MM.DD - 홈팀 (스코어) 원정팀. DB에 없으면 반드시 "※ H2H 업데이트 예정" 으로만 표기. 웹 검색 절대 금지.)
SUMMARY: (종합 분석. 존댓말로 3문장 이상. 반드시 [${seasonLabel} 전체 DB] 기준 수치만 활용하라. 다른 연도 수치 사용 절대 금지. 아래 금지 사항을 반드시 준수하라. ①"제공된 DB", "DB만 놓고 보면", "H2H DB가 없어", "상대전적은 반영하지 않았고", "웹 검색 결과상", "결장 근거가 제한적" 같은 분석 과정·출처·한계를 드러내는 표현 절대 금지. ②독자 입장에서 읽히는 깔끔한 전력 비교와 예측만 작성하라. ③양 팀의 ${seasonLabel} 전력 차이, 득점/수비 흐름, 주목 포인트 순서로 자연스럽게 서술하라.)

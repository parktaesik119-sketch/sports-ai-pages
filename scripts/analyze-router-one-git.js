...
async function savePost(savePath, aiText, match, dateShort, cat, dateOnly, h2hContent, aiHomeName, aiAwayName, homeRecentMatches = [], awayRecentMatches = [], h2hHistory = [], expectedScores = null, fotmobHomeLineupJson = '', fotmobAwayLineupJson = '', fotmobHomeFormationVal = '', fotmobAwayFormationVal = '', fotmobHomeCoachVal = '', fotmobAwayCoachVal = '') {
...
const winnerIsHome = homeNames.some(n =>
  ...
  const homeRecentJson = JSON.stringify(homeRecentMatches.slice(0, 5).map(m => ({
    ...
    result: (() => {
  const isHomeTeam = (TEAM_NAME_MAP[m.home] || m.home) === (TEAM_NAME_MAP[match.home] || match.home);
  ...
  const awayRecentJson = JSON.stringify(awayRecentMatches.slice(0, 5).map(m => ({
    ...
    score: (typeof m.homeScore === 'number' && typeof m.awayScore === 'number') ? `${m.homeScore}-${m.awayScore}` : (m.score || '-'),
    result: (() => {
  const isHomeTeam = (TEAM_NAME_MAP[m.home] || m.home) === (TEAM_NAME_MAP[match.away] || match.away);
  const my = isHomeTeam ? Number(m.homeScore) : Number(m.awayScore);
  const op = isHomeTeam ? Number(m.awayScore) : Number(m.homeScore);
  if (isNaN(my) || isNaN(op)) return '-';
  return my > op ? '🟢승' : my < op ? '🔴패' : '🟡무';
